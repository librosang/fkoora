...
// ---------------------------------------------------------------------------
// team page SEO (per-team URLs: /team/<id>/<name>)
// ---------------------------------------------------------------------------

const TEAM_SLUG_MAX = 60;

/** Slug of a team name in one language ("real-madrid" / "ريال-مدريد"). */
export function teamSlug(
  t: { nameEn?: string | null; nameAr?: string | null } | null | undefined,
  lang: Lang,
): string {
  const name = lang === "ar" ? t?.nameAr || t?.nameEn : t?.nameEn || t?.nameAr;
  let slug = slugifyText(name);
  if (slug.length > TEAM_SLUG_MAX) {
    slug = slug.slice(0, TEAM_SLUG_MAX).replace(/-+$/g, "");
  }
  return slug;
}

/** Path of a team page: "/team/<id>/<slug>". */
export function teamUrlPath(teamId: string, slug: string): string {
  const id = encodeURIComponent(teamId);
  return slug ? `/team/${id}/${encodeURIComponent(slug)}` : `/team/${id}`;
}

/** The two canonical URLs of a team (same rules as matchUrlPair). */
export function teamUrlPair(
  teamId: string,
  t: { nameEn?: string | null; nameAr?: string | null } | null | undefined,
): LangUrlPair {
  const slugAr = teamSlug(t, "ar");
  const slugEn = teamSlug(t, "en");
  const ar = teamUrlPath(teamId, slugAr);
  const en =
    slugAr === slugEn
      ? `${teamUrlPath(teamId, slugEn)}?lang=en`
      : teamUrlPath(teamId, slugEn);
  return { ar, en };
}

/** The canonical URL of a team in ONE language. */
export function teamUrlFor(
  teamId: string,
  t: { nameEn?: string | null; nameAr?: string | null } | null | undefined,
  lang: Lang,
): string {
  return lang === "en" ? teamUrlPair(teamId, t).en : teamUrlPair(teamId, t).ar;
}
...
function teamDisplayName(
  ...
  return (lang === "ar" ? t?.nameAr || t?.nameEn : t?.nameEn || t?.nameAr) || "";
}

/** <title> for a team page (same string on server and client). */
export function teamTitle(
  t: { nameEn?: string | null; nameAr?: string | null } | null | undefined,
  lang: Lang,
): string {
  const name = teamDisplayName(t, lang);
  if (lang === "ar") {
    return name
      ? `مباريات ${name} — النتائج والمواعيد والتشكيلة | ${APP_NAME_AR}`
      : `مباريات الفريق — النتائج والمواعيد | ${APP_NAME_AR}`;
  }
  return name
    ? `${name} Matches, Results & Fixtures | ${APP_NAME_EN}`
    : `Team Matches, Results & Fixtures | ${APP_NAME_EN}`;
}

/** Meta description for a team page. */
export function teamDescription(
  ...
  if (lang === "ar") {
    return name
      ? `نتائج ومواعيد مباريات ${name} بتوقيتك المحلي، آخر نتائج الفريق ومبارياته القادمة وتشكيلة اللاعبين — على ${APP_NAME_AR}.`
      : `نتائج ومواعيد مباريات الفريق مع التشكيلة والمباريات القادمة — على ${APP_NAME_AR}.`;
  }
  return name
    ? `${name} latest results and upcoming fixtures with kick-off times in your timezone, plus the full squad list — bilingual on ${APP_NAME_EN}.`
    : `Team results and upcoming fixtures with the full squad list — on ${APP_NAME_EN}.`;
}

/**
 * JSON-LD for a team page: a SportsTeam node, an ItemList of the team's
 * recent/upcoming SportsEvents (each links to its own match page in the SAME
 * language) and a BreadcrumbList.
 */
export function teamJsonLd(
  input: {
    teamId: string;
    team: { nameEn?: string | null; nameAr?: string | null; crestUrl?: string | null; shortNameEn?: string | null; code?: string | null };
  ...
  const base = siteUrl();
  const name = teamDisplayName(input.team, lang) || (lang === "ar" ? APP_NAME_AR : APP_NAME_EN);
  const altName = lang === "ar" ? input.team.nameEn || input.team.nameAr : input.team.nameAr || input.team.nameEn;
  const url = `${base}${teamUrlFor(input.teamId, input.team, lang)}`;

  const matches = [...input.recentMatches.slice(0, 5), ...input.upcomingMatches.slice(0, 5)];
    ...
    "@graph": [
      {
        "@type": "SportsTeam",
        name,
        alternateName: altName || undefined,
        sport: lang === "ar" ? "كرة القدم" : "Football",
        url,
      },
      {
        "@type": "ItemList",
        name:
          lang === "ar" ? `مباريات ${name}` : `${name} matches`,
        url,
        inLanguage: lang,
...
// ---------------------------------------------------------------------------
// player page SEO (per-player URLs: /player/<id>/<name>)
// ---------------------------------------------------------------------------

const PLAYER_SLUG_MAX = 60;

/** Slug of a player name in one language ("kylian-mbappe" / "كيليان-مبابي"). */
export function playerSlug(
  p: { nameEn?: string | null; nameAr?: string | null } | null | undefined,
  lang: Lang,
): string {
  const name = lang === "ar" ? p?.nameAr || p?.nameEn : p?.nameEn || p?.nameAr;
  let slug = slugifyText(name);
  if (slug.length > PLAYER_SLUG_MAX) {
    slug = slug.slice(0, PLAYER_SLUG_MAX).replace(/-+$/g, "");
  }
  return slug;
}

/** Path of a player page: "/player/<id>/<slug>". */
export function playerUrlPath(playerId: string, slug: string): string {
  const id = encodeURIComponent(playerId);
  return slug ? `/player/${id}/${encodeURIComponent(slug)}` : `/player/${id}`;
}

/** The two canonical URLs of a player (same rules as matchUrlPair). */
export function playerUrlPair(
  playerId: string,
  p: { nameEn?: string | null; nameAr?: string | null } | null | undefined,
): LangUrlPair {
  const slugAr = playerSlug(p, "ar");
  const slugEn = playerSlug(p, "en");
  const ar = playerUrlPath(playerId, slugAr);
  const en =
    slugAr === slugEn
      ? `${playerUrlPath(playerId, slugEn)}?lang=en`
      : playerUrlPath(playerId, slugEn);
  return { ar, en };
}

/** The canonical URL of a player in ONE language. */
export function playerUrlFor(
  playerId: string,
  p: { nameEn?: string | null; nameAr?: string | null } | null | undefined,
  lang: Lang,
): string {
  return lang === "en"
    ? playerUrlPair(playerId, p).en
    : playerUrlPair(playerId, p).ar;
}
...
function playerDisplayName(
  ...
  return (lang === "ar" ? p?.nameAr || p?.nameEn : p?.nameEn || p?.nameAr) || "";
}

/** <title> for a player page (same string on server and client). */
export function playerTitle(
  p: { nameEn?: string | null; nameAr?: string | null } | null | undefined,
  lang: Lang,
): string {
  const name = playerDisplayName(p, lang);
  if (lang === "ar") {
    return name
      ? `${name} — ملف اللاعب والإحصائيات والمسيرة | ${APP_NAME_AR}`
      : `ملف اللاعب — الإحصائيات والمسيرة | ${APP_NAME_AR}`;
  }
  return name
    ? `${name} Profile, Stats & Career | ${APP_NAME_EN}`
    : `Player Profile, Stats & Career | ${APP_NAME_EN}`;
}

/** Meta description for a player page. */
export function playerDescription(
  input: {
    player: { nameEn?: string | null; nameAr?: string | null };
    clubName?: string | null;
  },
  lang: Lang,
): string {
  const name = playerDisplayName(input.player, lang);
  ...
  if (lang === "ar") {
    return name
      ? `ملف ${name}${withClub}: المركز والرقم والعمر والطول والجنسية وسجل المسيرة الكامل بالأرقام لكل موسم — على ${APP_NAME_AR}.`
      : `ملف اللاعب مع سجل المسيرة الكامل بالأرقام لكل موسم — على ${APP_NAME_AR}.`;
  }
  return name
    ? `${name}${withClub} profile: position, shirt number, age, height, nationality and full career statistics per season — bilingual on ${APP_NAME_EN}.`
    : `Player profile with full career statistics per season — on ${APP_NAME_EN}.`;
}

/**
 * JSON-LD for a player page: a Person node (name, birthDate, nationality,
 * affiliation with the current club as SportsTeam) + a BreadcrumbList.
 */
export function playerJsonLd(
  ...
  const base = siteUrl();
  const name = playerDisplayName(input.player, lang) || (lang === "ar" ? APP_NAME_AR : APP_NAME_EN);
  const altName =
    lang === "ar" ? input.player.nameEn || input.player.nameAr : input.player.nameAr || input.player.nameEn;
  const url = `${base}${playerUrlFor(input.playerId, input.player, lang)}`;
  ...
  if (input.player.birthDate) person.birthDate = input.player.birthDate;
  const nationality =
    lang === "ar"
      ? input.player.nationalityAr || input.player.nationalityEn
      : input.player.nationalityEn || input.player.nationalityAr;
  if (nationality) person.nationality = { "@type": "Country", name: nationality };
  if (input.currentClub) {
    person.affiliation = {
      "@type": "SportsTeam",
      name: teamDisplayName(input.currentClub, lang),
      url: input.currentClub.id
        ? `${base}${teamUrlFor(input.currentClub.id, input.currentClub, lang)}`
        : undefined,
    };
  }
  if (input.player.imageUrl && input.player.imageUrl.startsWith("/")) {
    person.image = `${base}${input.player.imageUrl}`;
  }
